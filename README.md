...
Этот модуль содержит функцию которая принимает на вход путь до JSON-файла и возвращает список словарей с данными 
о финансовых транзакциях. Если файл пустой, содержит не список или не найден, функция возвращает пустой список.
Создан новый модуль под названием external_api:
Этот модуль содержит функцию которая которая принимает на вход транзакцию и возвращает сумму транзакции в рублях.
...
4. Version 0.5:
Создан новый модуль под названием read_transaction_csv_xlsx:

Этот модуль содержит функции, test_read_trans_csv.py и test_read_trans_xlsx.py которые принимают на вход путь до 
csv и Excel файлов соответственно содержащих информацию о транзакциях и возвращают список словарей этих транзакций

5. Version 0.6:
Создан новый модуль под названием search_trans:
Этот модуль содержит функции:

        а. trans_search, которая принимает список словарей с данными о банковских операциях и строку поиска, 
        а возвращает список словарей, у которых в описании есть данная строка.
        При реализации этой функции использовалась библиотека re для работы с регулярными выражениями.

        б. count_trans, которая принимает список словарей с данными о банковских операциях и список категорий операций, 
...
Создан модуль main содержащий функцию main которая отвечает за основную логику проекта и связывает функциональности 
между собой.

Создан новый модуль filter содержащий функцию filter_by_curr_csv имеющую такой же функционал как у filter_by_currency 
в модуле generators, адаптированный для CSV и EXCEL файлов.

Адаптированы старые функции для решения новых задач.
...
Обновление 0.5:
1. Был создан модуль test_read_trans_csv в директории tests и были произведены следующие тесты:
- имитация успешного чтения CSV-файла и проверка, что результат соответствует ожидаемому списку словарей.
- проверка на то, что функция возвращает пустой список, если имя файла пустое
- симуляция случая, когда файл не найден, и проверка, что возвращается пустой список.
2. Был создан модуль test_read_trans_xlsx в директории tests и были произведены следующие тесты:
- имитация успешного чтения Excel-файла и проверка, что результат соответствует ожидаемому списку словарей.
- проверка на то, что функция возвращает пустой список, если имя файла пустое.
- проверка на то, что функция возвращает пустой список, если файл не найден.

Обновление 0.6:
1. Изменена логика многих тестов или они были разбиты на различные модули.
2. Создан модуль test_search_trans в директории tests и были произведены следующие тесты:
- проверка, что функция возвращает транзакции, в описании которых содержится искомая строка.
- убеждение в том, что поиск нечувствителен к регистру.
- проверка случая, когда ни одна транзакция не соответствует искомой строке.
- проверка поведения функции при пустом списке транзакций.
3. Создан модуль test_count_search в директории tests и были произведены следующие тесты:
- проверка, правильно ли функция count_trans считает транзакции по категориям.
- проверка, как функция ведет себя при пустых входных списках транзакций и категорий.
4. Создан модуль test_filter в директории tests содержащий различные ситуации:
